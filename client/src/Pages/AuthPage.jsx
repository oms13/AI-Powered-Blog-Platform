...
const AuthPage = ({ mode }) => {
    ...
    const toggleMode = () => {
        setIsLogin(!isLogin);
        setError('');
        // Optional: clear form data when switching modes
            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.08)] w-full max-w-[440px] p-8 sm:p-10 relative z-10 border border-white">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {!isLogin && (
                        <div className="relative group">
                            ...
                            <input
                                ...
                                disabled={isLoading}
                                className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white outline-none transition-all text-gray-900 placeholder-gray-400 font-medium disabled:opacity-60"
                            />
                        </div>
                    )}
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            onChange={handleChange}
                            disabled={isLoading}
                            className="w-full pl-11 pr-12 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white outline-none transition-all text-gray-900 placeholder-gray-400 font-medium disabled:opacity-60"
                        />
                        <button
                            type="button"
                            onClick={() => setShowPassword(!showPassword)}
                            disabled={isLoading}
                            className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-gray-600 focus:outline-none disabled:opacity-60"
                        >
                    ...
                    {!isLogin && (
                        <div className="relative group">
                            ...
                            <select
                                ...
                                onChange={handleChange}
                                disabled={isLoading}
                                className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white outline-none transition-all text-gray-900 appearance-none cursor-pointer font-medium disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 mt-2 disabled:opacity-70 disabled:cursor-not-allowed group"
                    >
                        {isLoading ? (
                            <>
                ...
                <div className="mt-8 text-center text-[15px] text-gray-600">
                    ...
                    <button
                        ...
                        onClick={toggleMode}
                        disabled={isLoading}
                        className="font-bold text-indigo-600 hover:text-indigo-700 transition-colors disabled:opacity-60"
                    >
